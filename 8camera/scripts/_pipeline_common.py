...
_pipeline_common.py
--------------------
Shared plumbing for run_pipeline_8camera.py.
...
layout they were designed for, but 8camera/ keeps its own data, output and
config alongside those of 4camera/ and the original 8-shot rig. This bridges
that gap: temporarily swap the project's data/, output/ and config.yaml for
8camera's own copies (via a directory junction on Windows, a symlink
elsewhere), run the numbered scripts unchanged, then swap back - restored
even if a step raises, the setup itself is interrupted, or the whole process
is killed outright (self-heals on the next run - see _heal_stale_backup).

This is a straight port of 4camera/scripts/_pipeline_common.py - see that
file's comments for the reasoning behind each piece. Kept as a separate copy
rather than a shared import because 4camera/ and 8camera/ are independent,
self-contained rig folders by design (see each one's README).
"""
...
EIGHT_CAM_ROOT = os.path.dirname(SCRIPT_DIR)          # 8camera/
PROJECT_ROOT = os.path.dirname(EIGHT_CAM_ROOT)          # Kinefeet2_POC/
...
def versions_with_data():
    ...
    for version in ("full_body", "waist_down"):
        version_dir = os.path.join(DATA_8CAM_PATH, version)
...
_BORROWED_MARKER_NAME = ".kinefeet_borrowed_by_8camera_pipeline"
...
def _heal_stale_backup(path):
    ...
    the fresh backup collides with the stale one, or, worse, silently backs up
    the *borrowed* copy, burying the real data/config.yaml under it for good.
    """
...
def borrowed_project_paths(output_8cam_path):
    """Point the project's config.yaml / data/ / output/ at 8camera's own copies.

    Crash-safety covers both directions: a step raising inside the ``with``
    block restores everything via the ``finally`` below, and in case borrowing
    itself is interrupted (the process killed outright, not a normal Python
    exception - see ``_heal_stale_backup``) whatever backups this attempt
    already took are rolled back before the error propagates, and a still-stale
    one from an *earlier* interrupted run is healed before a new attempt even
    starts.
    """
    ...
    try:
        ...
        shutil.copy2(CONFIG_8CAM_PATH, PROJECT_CONFIG_PATH)
        os.makedirs(output_8cam_path, exist_ok=True)
        _link_or_copy(DATA_8CAM_PATH, PROJECT_DATA_PATH)
        _link_or_copy(output_8cam_path, PROJECT_OUTPUT_PATH_TEMPLATE)
...
def print_photo_naming():
    ...
    print("  02.jpg = 45 deg")
    print("  03.jpg = 90 deg (right side)")
    print("  04.jpg = 135 deg")
    print("  05.jpg = 180 deg (back)")
    print("  06.jpg = 225 deg")
    print("  07.jpg = 270 deg (left side)")
    print("  08.jpg = 315 deg")
